...
//                        echo "Aggiornamento fallito";
        ...
        $string .= "<form action='generiPreferiti.php' method='post'><fieldset><legend>Generi preferiti</legend><ul>";

        foreach ($generi as $key => $genere) {
            $string .= "<li><input id='modGenere" . htmlentities(preg_replace("/\s|\&/", "_", $genere), ENT_QUOTES, "UTF-8") . "' name='genere" . htmlentities(preg_replace("/\s|\&/", "_", $genere), ENT_QUOTES, "UTF-8") . "' type='checkbox'" . (in_array($genere, $preferenze) ? " checked='true'" : "") . "/><label for='modGenere" . htmlentities(preg_replace("/\s|\&/", "_", $genere)) . "'>" . htmlentities($genere, ENT_QUOTES, "UTF-8") . "</label></li>";
        }
